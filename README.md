...
- `/查询全局印象` 每页最多即时查询 20 位用户；查询失败时名称列留空，ID 列仍正常显示

## 命令

| 命令 | 权限 | 说明 |
|------|------|------|
| `/查询印象` | 所有成员 | 查看自己的好感度与情感维度 |
| `/查询印象 @用户` | Bot管理员 | 查看指定用户的好感度与情感维度 |
| `/查询全局印象 [页码]` | Bot管理员 | 好感度排行榜（T2I 分页渲染） |
| `/修改印象 @用户 数值` | Bot管理员 | 直接设置用户好感度 |
| `/修改情感 @用户 维度 数值` | Bot管理员 | 修改单个或全部情感维度 |
| `/清空印象 @用户` | Bot管理员 | 清空指定用户数据（二次确认+自动备份） |
...
- **lazy 求值**：所有读位点（LLM 注入、`/查询印象`、`/查询全局印象`、裁决结算）调用纯函数实时计算衰减值，不写库——避免无意义的 UPDATE
