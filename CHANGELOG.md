...
- **排行榜隔断竖条跑位**：`/emotion list` 每行的柱形区原先用 `flex: 1 1 auto` 撑满、主导情感文字区按内容定宽，于是 top3 不足三个（缺情绪）的用户那根隔断竖条会左右浮动，多人同页时参差不齐；改为柱形区按内容定宽、文字区吸收剩余空间，竖条位置对所有用户一致。

### 变更

- **测试精简**：按「改错一眼看不出」的判据重审全部用例，50 → 48 项（Python 45 + 前端 3）。删掉只验证字符串拼接形状的 `test_interaction_section.py`、测未被生产消费的策略表的用例，以及三处「变异后照样通过」的等价输入；合并同不变量的重复用例；新增 `tests/test_render_custom_t2i.py` 补上本批改动函数的覆盖。共享桩改为 `tests/_shared.py`，由各测试文件 importlib 按路径加载。
- **删除死代码**：`TaskSupervisor.closing`、`pending_count`，以及从未在生产使用过的 `persistent` 任务通道（`spawn(persistent=True)`，关闭时立即取消的那一支）一并移除；任务收尾现在只有「给完成窗口、超时再取消」一种语义。

测试：行为级 48 项（Python 45 + 前端 3）；本轮对 69 个变异点做了验证，67 个被拦住，其余 2 个经核实为等价变异。
...
- **关系判定与拒绝口吻**：静态系统提示块新增小节（紧接「关系档位结构」之后）——关系等级与情感状态以注入标签为权威，没有标签时按中间友好态处理；用户口头自称不算越级、不推动升级，反向贬低也不推动降级；对话氛围可实时校准温度但等级不变；拒绝的口吻随关系等级变化（陌生冷而短、友好讲清理由、亲近先说明在乎再拒绝），结论不变。
